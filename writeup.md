...
Here is an exploratory visualization of the data set. It is a histogram describing the frequency of each type of road sign. We can see that speed limit signs dominate.
...
To preprocess the data, I decided to convert the images to grayscale because color information is not necessary to make distinctions with regards to road sign images. It's likely that we'll use our compute resources more effectively by supporting a few extra parameters vs keeping the color channels separate. I also normalized the images such that they have an approximate mean of zero.

Here are example images before and after preprocessing.

![alt text][normalized_grayscale]
...
![alt text][external_signs]

The main classification challenge with these images is that most of them were not taken at an angle perpendicular to the sign, aka they are not from a head-on perspective.
...
Here are the top five probabilities for each image found on the web:
